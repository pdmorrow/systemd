...
EOF
    ...
    # Ensure the OnFailure dependency list for failure-handler@test0.service
    # is empty since we cleared it out via a drop-in.
    check_ok failure-handler@test0.service OnFailure ""
    ...
    # Start the service and make sure the nothing runs as a consequence of
    # the service failing (it fails by definition).
    set +e
...
test_basic_onfailure_dep_removal() {
    ...
    # A service which will be called via OnFailure from test.service, as
